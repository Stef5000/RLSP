...
void DQN_Network::save_to_stream(std::ofstream &file) const {
    file.write(reinterpret_cast<const char*>(&input_size), sizeof(input_size));
    file.write(reinterpret_cast<const char*>(&hidden_size1), sizeof(hidden_size1));
    file.write(reinterpret_cast<const char*>(&hidden_size2), sizeof(hidden_size2));
    ...
    file.write(reinterpret_cast<const char*>(w3.data()), sizeof(float) * w3.size());
    ...
    file.write(reinterpret_cast<const char*>(b3.data()), sizeof(float) * b3.size());
}

void DQN_Network::load_from_stream(std::ifstream &file) {
    int li, lh1, lh2, lo;
    file.read(reinterpret_cast<char*>(&li), sizeof(li));
    file.read(reinterpret_cast<char*>(&lh1), sizeof(lh1));
    file.read(reinterpret_cast<char*>(&lh2), sizeof(lh2));
    ...
    file.read(reinterpret_cast<char*>(w3.data()), sizeof(float) * w3.size());
    file.read(reinterpret_cast<char*>(b1.data()), sizeof(float) * b1.size());
    file.read(reinterpret_cast<char*>(b2.data()), sizeof(float) * b2.size());
    file.read(reinterpret_cast<char*>(b3.data()), sizeof(float) * b3.size());
}
...
void DQN::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_observation", "obs"), &DQN::set_observation);
...
DQN::DQN() :
    ...
    episode_count(0)
{
    // set Eigen threads once
    unsigned int hw = std::thread::hardware_concurrency();
    if (hw == 0) hw = 1;
    Eigen::setNbThreads(static_cast<int>(hw));
}
